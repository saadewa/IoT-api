...
class DeviceController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        return response()->json([
            "message" => "Device telah di tambahkan."
        ], 201);
    }
    ...
    public function update(Request $request, string $id)
    {
        if (Device::where('id', $id) ->exists()) {
            ...
            $request->nama_device;
            $device->nilai_min = is_null($request->nilai_min) ?
            $device->nilai_min : $request->nilai_min;
            $device->nilai_max = is_null($request->nilai_max) ?
            $device->nilai_max : $request->nilai_max;
            $device->nilai = is_null($request->nilai) ?
            $device->nilai : $request->nilai;
            $device->save();
            return response()->json([
